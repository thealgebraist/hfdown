...
def main():
    ...
    parser.add_argument("--num-large", type=int, default=0)
    parser.add_argument("--large-size-mb", type=float, default=100.0)
    ...
    # Large files: either explicit list or generated count
    large_sizes = args.large_sizes_mb
    if not large_sizes and args.num_large > 0:
        # Generate N files of approximately large_size_mb
        for _ in range(args.num_large):
            # Variance +/- 20%
            size = args.large_size_mb * random.uniform(0.8, 1.2)
            large_sizes.append(size)

    for i, size_mb in enumerate(large_sizes):
        filename = f"large_file_{i}_{int(size_mb)}mb.bin"
        print(
            f"Generating {filename} ({size_mb:.2f} MB) with random bytes: {args.random_large}..."
        )
        generate_file(
            os.path.join(model_dir, filename), size_mb, random_data=args.random_large
        )
        files_info.append(
            {"path": filename, "size": int(size_mb * 1024 * 1024), "type": "file"}
        )
    ...
    for i in range(args.num_small):
        ...
        generate_file(
            os.path.join(model_dir, filename), size_kb / 1024.0, random_data=True
        )
        files_info.append(
            {"path": filename, "size": int(size_kb * 1024), "type": "file"}
        )
